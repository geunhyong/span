...
def evaluate_persistence_naive(label, current_return, y_test):
    """
    현재 주 삼성전자 로그수익률을
    다음 주 삼성전자 로그수익률의 예측값으로 사용하는 Naive Baseline.

    예측식:
        predicted_return(t+1) = samsung_return(t)
    """
    y_pred = np.asarray(current_return)

    if len(y_pred) != len(y_test):
        raise ValueError(
            "Naive 예측값과 테스트 타깃의 길이가 일치하지 않습니다."
        )
...
def save_prediction_outputs(
    ...
    """
    삼성전자 단일 타깃 테스트 구간의 실제값과
    모델별 예측값, 방향 및 적중 여부를 CSV로 저장한다.
    ...
    prediction_arrays = {
        "Naive": np.asarray(y_pred_naive),
        "Price_Only": np.asarray(y_pred_price_only),
        "Model_A": np.asarray(y_pred_a),
        "Model_B": np.asarray(y_pred_b),
        "Model_C": np.asarray(y_pred_c),
    }

    for model_name, prediction_array in prediction_arrays.items():
        if len(prediction_array) != len(y_true):
            raise ValueError(
                f"{model_name} 예측값 길이와 실제값 길이가 일치하지 않습니다."
            )
    ...
    def classify_direction(values):
        ...
        return np.where(
            values > 0,
            "UP",
            np.where(values < 0, "DOWN", "FLAT"),
        )

    actual_direction = classify_direction(y_true)
    ...
    prediction_df = pd.DataFrame(
        {
            ...
            "Naive_Pred_Log_Return": prediction_arrays["Naive"],
            "Price_Only_Pred_Log_Return": prediction_arrays["Price_Only"],
            "Model_A_Pred_Log_Return": prediction_arrays["Model_A"],
            "Model_B_Pred_Log_Return": prediction_arrays["Model_B"],
            "Model_C_Pred_Log_Return": prediction_arrays["Model_C"],
            "Actual_Direction": actual_direction,
        }
    )

    for column_prefix, prediction_array in prediction_arrays.items():
        prediction_df[f"{column_prefix}_Pred_Direction"] = (
            classify_direction(prediction_array)
        )

        prediction_df[f"{column_prefix}_Match"] = (
            (np.sign(y_true) == np.sign(prediction_array))
            & (np.sign(y_true) != 0)
        )
    ...
    print(
        f"모델별 예측 상세 CSV 저장 완료: {prediction_path}"
    )
...
def main():
    ...
    # 삼성로그수익률 테스트 구간
    samsung_current_return_test = (
    df_final["Samsung_Log_Return"]
    .iloc[split_idx:])

    results = []
    prediction_map = {}

    # naive persistence baseline: 다음 주 수익률 예측값 = 이번 주 수익률
    naive_metrics, y_pred_naive = evaluate_persistence_naive(
    ...
    prediction_map["Naive Persistence Baseline"] = y_pred_naive
